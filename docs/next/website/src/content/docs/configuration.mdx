...
{/* fork: context tabs */}
Group Spaces into context tabs with a workspace metadata token. Set `context_token` to the token key and a tab row appears across the top of the window, above the sidebar and tab bar: `+`, the default context, every known context, and `all`. Spaces without the token, including every Space that already exists when the setting is enabled, belong to `default_context`. That tab cannot be renamed; it is drawn only while a Space without the token exists or while it is selected, so it disappears on its own once everything has a context and returns when a new untagged Space shows up. The active tab filters the Space and Agent panels, navigate mode, `prefix+1..9`, next/previous workspace, and agent cycling. All three keys are unset by default.
...
[keys]
next_context = "prefix+ö"
previous_context = "prefix+ä"
move_workspace_to_context = "prefix+shift+m"
```

Click a tab to activate it, click `+` to create a context, or right-click a tab to rename or remove it; removing a context clears the token on its Spaces. Switching to a tab focuses the Space you last used under that context (or its first Space) unless the focused Space already belongs to it. A tab with no Space on the current machine shows an empty pane area instead of the server's focused Space, and typed input goes nowhere until a Space is created or moved there. Right-click a Space and choose "Move to context..." to move it, or "New context..." to create a context and move the Space in one step. A Space created while a named tab is active gets that context. Scripts and plugins can set the same token:

```bash
herdr workspace report-metadata <workspace_id> --source my-hook --token context=client
```

Known contexts, the active tab, and a per-Space backup of the token live in the client state file, so they survive reattaching. The server does not persist metadata tokens across restarts; after a restart the client re-reports the backed-up context for every Space that came back without one. The tab row is drawn locally, so it works the same with `herdr --remote` and saved machines. With more than one machine it replaces the `machines` heading and the active tab applies to every machine.
