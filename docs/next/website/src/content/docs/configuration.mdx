...
{/* fork: context tabs */}
Group Spaces into context tabs with a workspace metadata token. Set `context_token` to the token key and the Space heading becomes a tab row: `+`, the default context, every known context, and `all`. Spaces without the token, including every Space that already exists when the setting is enabled, belong to `default_context`, which cannot be renamed or removed. The active tab filters the Space and Agent panels, navigate mode, `prefix+1..9`, next/previous workspace, and agent cycling. All three keys are unset by default.
